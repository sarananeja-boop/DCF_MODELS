...
def generate_sensitivity_grid(metrics: Dict[str, Any], wacc_data: Dict[str, float], stock_data: Dict[str, Any], market_profile: Dict[str, Any], terminal_growth: float = 0.025, projection_years: int = 5, grid_size: int = 9) -> Dict[str, Any]:
    """
    Generates a 2D sensitivity grid for Revenue Growth vs. WACC.
    """
    avg_rev_growth = metrics.get('avg_rev_growth', 0.0)
    avg_ebit_margin = metrics.get('avg_ebit_margin', 0.0)
    ...
    for w in wacc_range:
        ...
        for g in growth_range:
            try:
                # Use base_wacc calculation for terminal/pv if needed inside run_dcf, 
                # but we override discount_rate here
                dcf_result = run_dcf(
                    start_growth=g,
...
def generate_margin_sensitivity_grid(metrics: Dict[str, Any], wacc_data: Dict[str, float], stock_data: Dict[str, Any], market_profile: Dict[str, Any], terminal_growth: float = 0.025, projection_years: int = 5, grid_size: int = 9) -> Dict[str, Any]:
    """
    ...
    # Generate ranges
    margin_range = np.linspace(avg_ebit_margin - 0.08, avg_ebit_margin + 0.08, grid_size)
    raw_wacc_range = np.linspace(base_wacc - 0.03, base_wacc + 0.03, grid_size)
    ...
    return {
        ...
        "wacc_range": wacc_range.tolist(),
        "price_grid": price_grid,
        "base_margin": float(avg_ebit_margin),
        "base_wacc": float(base_wacc)
    }
