...
export default function MacroDisplay({ data }) {
  ...
  return (
    <div className="bg-navy-800 rounded-lg p-4 border border-navy-600 shadow-lg">
      ...
      <div className="space-y-3">
        ...
        <div className="flex justify-between items-center pb-2 border-b border-navy-700">
          ...
          <div className="text-right">
            <span className="text-sm font-mono text-white">{(macro.risk_free_rate * 100).toFixed(2)}%</span>
        ...
        <div className="flex justify-between items-center pb-2 border-b border-navy-700">
          ...
          <span className="text-sm font-mono text-white">{(macro.market_return * 100).toFixed(1)}%</span>
        </div>

        <div className="flex justify-between items-center">
          <span className="text-sm text-text-secondary">Tax Rate</span>
          <span className="text-sm font-mono text-white">{(macro.tax_rate * 100).toFixed(2)}%</span>
        </div>
      </div>
    </div>
  );
